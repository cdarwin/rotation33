...
- Our **Release** = a Discogs **master release** (the album across pressings).
- Our **Instance** = a single owned physical copy = a Discogs **collection
  instance**.
...
`folder_id`. The library paginates (~50/page) and handles rate-limit backoff.
...
`item.release.data` — the Discogs `basic_information` dict, present in the
listing payload with **no extra HTTP request**.
...
**Implication:** sync can populate Release identity and album metadata directly
from the collection listing. No N+1 fetch per release.

### 2. Identity: Instance = `instance_id`, Release = `master_id`

- `item.instance_id` is the collection-unique per-copy holding id (~2.1e9 range).
  It is the natural **Instance** primary key.
- `item.id` is the Discogs **release** id (a specific pressing) — *not* stable
  across pressings of the same album.
...
Release `37907304` ("Hookups & Heartaches", an indie self-release) has no master:
...
The `master_id` **key is always present**; its value is `0` when there is no
master (not absent). `master_url` is `None`.
...
Distinct release ids, distinct instance ids, one `master_id`. In our model:
**one Release, two Instances.** The pressing-specific detail (color) lives in
...
a **third** holding the owner didn't expect — see below.
...
`formats[].name` (`"Vinyl"` / `"CD"` / `"Cassette"`) is in the listing payload —
the filter is cheap, no extra fetch.

**Filter at the Instance level, not the master level:**

- Keep an Instance iff `any(f["name"] == "Vinyl" for f in basic_information["formats"])`.
- A Release exists iff it has ≥1 kept (vinyl) Instance.
...
(e.g. an LP+CD box set → `["Vinyl", "CD"]`). The `any(... == "Vinyl")` rule keeps
those, which is the desired behavior for a vinyl collection.
...
| Format scope | Keep Instance iff any format name is `"Vinyl"`; Release exists iff ≥1 vinyl Instance |
| Cost | Full sync = one paginated collection walk; no N+1 |
...
- Multi-format-per-release (LP+CD in one release) was not observed in this
...
Spike script is untracked (gitignored). See the `EDGE CASES` and `SUMMARY`
